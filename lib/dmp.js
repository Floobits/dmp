    ...
    switch (diffs[pointer][0]) {
      ...
      case DIFF_EQUAL:
        ...
        if (count_delete >= 1 && count_insert >= 1) {
          ...
          a = this.diff_main(text_delete, text_insert, false, deadline);
...
diff_match_patch.prototype.diff_cleanupSemanticLossless = function (diffs) {
  ...
  function diff_cleanupSemanticScore_(one, two) {
    ...
    const whitespace1 = nonAlphaNumeric1 && char1.match(diff_match_patch.whitespaceRegex_);
    const whitespace2 = nonAlphaNumeric2 && char2.match(diff_match_patch.whitespaceRegex_);
    const lineBreak1 = whitespace1 && char1.match(diff_match_patch.linebreakRegex_);
    const lineBreak2 = whitespace2 && char2.match(diff_match_patch.linebreakRegex_);
    const blankLine1 = lineBreak1 && one.match(diff_match_patch.blanklineEndRegex_);
    const blankLine2 = lineBreak2 && two.match(diff_match_patch.blanklineStartRegex_);
    ...
    switch (diffs[x][0]) {
      case DIFF_INSERT:
        try {
          ...
          throw new Error("Illegal bytes in diff_toDelta: " + diffs[x][1]);
        }
        break;
      case DIFF_DELETE:
 ...
 */
diff_match_patch.prototype.patch_apply = function (patches, text, return_positions) {
  if (patches.length === 0) {
    return [text, []];
  }
  ...
  for (let x = 0; x < patches.length; x++) {
    ...
    const np_len = nullPadding.length;
    ...
    const too_close = (position[0] + position[2].length) - (text.length - 2 * np_len);
    if (too_close > 0) {
...
diff_match_patch.patch_obj.prototype.toString = function () {
  ...
  for (let x = 0; x < this.diffs.length; x++) {
    ...
    try {
      ...
      // TODO: handle this error more gracefully
      throw new Error("Illegal bytes in patch_obj.toString: " + this.diffs[x][1]);
    }
  }
  return text.join("").replace(/%20/g, " ");
};
